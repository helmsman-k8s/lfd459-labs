...
1. Connect to the **controller** node. Verify Python 3 is installed.
    ...
    ```bash
    guru@controller:~$ sudo apt-get -y install python3
    ```

    It should report already installed on Ubuntu 22.04.

2. Copy the chapter lab files to your working directory.

    ```bash
    guru@controller:~$ cp ~/lfd459/ch03-build/* .
    guru@controller:~$ ls
    ```
...
4. Copy `simple.py` into the app1 directory and review it.
    ...
    ```bash
    guru@controller:~/app1$ cp ../simple.py .
    guru@controller:~/app1$ cat simple.py
    ```
...
5. Make the script executable and run it briefly to verify it works. Use **Ctrl-C** to stop it after 15-20 seconds.
    ...
    guru@controller:~/app1$ ./simple.py
...
6. Verify the output file was created with hostname and timestamp entries.
    ...
    ```bash
    guru@controller:~/app1$ cat date.out
    2026-03-18 19:27:50
    controller
    2026-03-18 19:27:55
    controller
...
7. Copy the `Dockerfile` into the app1 directory and review it.
    ...
    ```bash
    guru@controller:~/app1$ cp ../Dockerfile .
    guru@controller:~/app1$ cat Dockerfile
    ```
...
8. Install Podman. Podman is a daemonless container tool compatible with Docker syntax.
    ...
    ```bash
    guru@controller:~/app1$ sudo apt-get install -y podman
    ```
...
10. Verify the new image appears in the local image list.
    ...
    ```bash
    guru@controller:~/app1$ sudo podman images
...
11. Run the container image with Podman. The script will run silently in the foreground. Wait 15 seconds then use **Ctrl-C** to stop it.
    ...
    ```bash
    guru@controller:~/app1$ sudo podman run localhost/simpleapp
    ^CTraceback (most recent call last):
      File "./simple.py", line 24, in <module>
        time.sleep(5)
    KeyboardInterrupt
...
12. Locate the `date.out` file that Podman created inside the container's overlay filesystem. Note the container hostname is a random hash, not `controller`.
    ...
    ```bash
    guru@controller:~/app1$ sudo find / -name date.out
...
13. View the contents of the container's `date.out` file. Use the full path returned by the find command above.
    ...
    ```bash
    guru@controller:~/app1$ sudo tail /var/lib/containers/storage/overlay/<hash>/diff/date.out
    2026-03-18 19:32:20
    b8cec851efca
    2026-03-18 19:32:25
    b8cec851efca
...
1. Return to the home directory and create the local registry using the provided YAML. This deploys an nginx frontend and a registry backend, along with the necessary PersistentVolumes and PVCs.

    ```bash
    guru@controller:~/app1$ cd $HOME
    guru@controller:~$ kubectl create -f easyregistry.yaml
...
3. Verify the registry is responding.
    ...
    ```bash
    guru@controller:~$ curl 10.97.40.62:5000/v2/_catalog
    {"repositories":[]}
    ```
...
4. Configure containerd and Podman on the **controller** to use the local registry over HTTP. Run the setup script (note the dot-space to source it and export the `$repo` variable into your shell).
    ...
    ```bash
    guru@controller:~$ chmod +x local-repo-setup.sh
    guru@controller:~$ . ./local-repo-setup.sh
    Configuring local repo, Please standby
    ...
    Local Repo configured, follow the next steps
    ```

    This script configures `/etc/containerd/config.toml` to mirror from `http://10.97.40.62:5000` and exports `repo=10.97.40.62:5000`.

5. Pull a small test image from Docker Hub, tag it for the local registry, and push it.

    ```bash
    guru@controller:~$ sudo podman pull docker.io/library/alpine
    guru@controller:~$ sudo podman tag alpine $repo/tagtest
    guru@controller:~$ sudo podman push $repo/tagtest
...
6. Remove the local cached copies of the images, then pull from your local registry to verify it works end-to-end.
    ...
    ```bash
    guru@controller:~$ sudo podman image rm alpine
    guru@controller:~$ sudo podman image rm $repo/tagtest
    guru@controller:~$ sudo podman pull $repo/tagtest
...
7. Connect to **worker1** in a separate terminal. Install Podman and configure the local registry on the worker node the same way.
    ...
    ```bash
    guru@worker1:~$ sudo apt-get install -y podman
    guru@worker1:~$ cp ~/lfd459/ch03-build/local-repo-setup.sh .
    guru@worker1:~$ chmod +x local-repo-setup.sh
    guru@worker1:~$ . ./local-repo-setup.sh
    ```

    Verify the worker can pull from the registry.

    ```bash
    guru@worker1:~$ sudo podman pull $repo/tagtest
    Trying to pull 10.97.40.62:5000/tagtest:latest...
    ...
    ```
...
8. Return to the **controller**. Tag and push the `simpleapp` image to the local registry.
    ...
    ```bash
    guru@controller:~$ cd app1
    guru@controller:~/app1$ sudo podman tag simpleapp $repo/simpleapp
    guru@controller:~/app1$ sudo podman push $repo/simpleapp
    Getting image source signatures
...
9. Verify both images appear in the registry catalog from the controller.
    ...
    ```bash
    guru@controller:~/app1$ curl $repo/v2/_catalog
...
10. Deploy the simpleapp from the local registry and scale it to 6 replicas.
    ...
    ```bash
    guru@controller:~/app1$ kubectl create deployment try1 --image=$repo/simpleapp
    deployment.apps/try1 created

    guru@controller:~/app1$ kubectl scale deployment try1 --replicas=6
    deployment.apps/try1 scaled

    guru@controller:~/app1$ kubectl get pod -o wide
...
11. On **worker1**, verify the simpleapp containers are running via crictl.
    ...
    ```bash
    guru@worker1:~$ sudo crictl config \
    --set runtime-endpoint=unix:///run/containerd/containerd.sock \
    --set image-endpoint=unix:///run/containerd/containerd.sock

    guru@worker1:~$ sudo crictl ps | grep simple
    ```

12. Return to the **controller**. Save the try1 deployment as a YAML file.

    ```bash
    guru@controller:~/app1$ kubectl get deployment try1 -o yaml > simpleapp.yaml
    ```

13. Delete and recreate the deployment from the saved YAML. Verify 6/6 replicas are running.

    ```bash
    guru@controller:~/app1$ kubectl delete deployment try1
    deployment.apps "try1" deleted

    guru@controller:~/app1$ kubectl create -f simpleapp.yaml
    deployment.apps/try1 created
...
5. Exec into one pod and create the health check file.
    ...
    ```bash
    guru@controller:~/app1$ kubectl exec -it try1-9869bdb88-rtchc -- /bin/bash
...
7. Use a `for` loop to touch `/tmp/healthy` in all remaining pods at once.
    ...
    ```bash
    guru@controller:~/app1$ for name in $(kubectl get pods -l app=try1 -o name); \
...
9. Edit `simpleapp.yaml` again to add a `livenessProbe` and a second sidecar container (`goproxy`). The `goproxy` container exposes port 8080 and will be used to test both a readiness and a liveness probe.

    Find the end of the simpleapp container definition (near `terminationMessagePolicy: File`) and add the goproxy container after it. The dash for `- name: goproxy` must align with the dash of the simpleapp container.
    ...
    !!! tip
        The `edited-later-simpleapp.yaml` file shows the complete deployment with both containers configured, including this sidecar. Use it as a reference for indentation.
...
11. Check the pods. Each pod now has two containers (`READY 1/2` or `0/2`). The goproxy container starts, but simpleapp is not ready again because `/tmp/healthy` is missing.
...
12. Create the health check file in all pods. Specify the `-c simpleapp` flag to target the correct container.
...
13. Wait a minute and verify all pods show `2/2 Running`.
...
14. Examine the events for one of the pods. Note the `Readiness probe failed` warnings from before the file was created.

    ```bash
    guru@controller:~/app1$ kubectl describe pod try1-76cc5ffcc6-4rjvh | tail -20
    ```

    Look for `Warning  Unhealthy` events showing `cat: /tmp/healthy: No such file or directory`.

15. Confirm both containers in the pod show `State: Running` and `Ready: True`.

    ```bash
    guru@controller:~/app1$ kubectl describe pod try1-76cc5ffcc6-4rjvh | grep -E 'State|Ready'
    State:          Running
    Ready:          True
    State:          Running
    Ready:          True
...
1. Using the three documentation sources allowed during the CKAD exam, find and bookmark working YAML examples for `livenessProbe`, `readinessProbe`, and multi-container pods.

2. Deploy a new nginx webserver. Add both a `livenessProbe` and a `readinessProbe` on port 80. Test that both probes and the webserver are working.

3. Use `build-review1.yaml` to create a broken deployment. Fix it so both containers are running and in a `READY` state.

    ```bash
    guru@controller:~/app1$ kubectl create -f ~/build-review1.yaml
    ```
...
4. Once fixed, access the default nginx page and verify the GET request appears in the container log.
    ...
    ```bash
    guru@controller:~$ curl http://<pod-ip>
    guru@controller:~$ kubectl logs <pod-name> -c brokenapp
    ```
...
5. Clean up all resources created in this review before moving on.

    ```bash
    guru@controller:~$ kubectl delete deployment try1 nginx registry break2 --ignore-not-found
    guru@controller:~$ kubectl delete svc nginx registry --ignore-not-found
    guru@controller:~$ kubectl delete pvc nginx-claim0 registry-claim0 --ignore-not-found
    guru@controller:~$ kubectl delete pv vol1 vol2 --ignore-not-found
    ```
